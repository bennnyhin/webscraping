import bs4, sys, urllib.error, requests
from urllib.request import urlopen as uReq
from bs4 import BeautifulSoup as soup


def amazon_search(search_query):
    my_url = "https://www.amazon.ca/s?k=" + search_query
    print (my_url)
    try:
        uClient = uReq(my_url)
    except urllib.error.HTTPError:
        return sys.exit("request not able to be made")
    else:
        page_html = uClient.read()
        page_soup = soup(page_html, "html.parser")
        return page_soup

page_soup = amazon_search("watch")
